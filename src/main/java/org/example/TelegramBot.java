...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private static final List<String> defaultButtons = List.of(ADD_EXPENSE_BTN, SHOW_CATEGORIES_BTN, SHOW_EXPENSES_BTN);

    @Override
    public String getBotUsername() {
        return "TetrisAkkkkBot";
    }
    ...
    private void handleAwaitsCategory(Message incomingMessage, ChatState currentChat) {
        String incomingText = incomingMessage.getText().toLowerCase();
        var capitalizedText = incomingText.substring(0, 1).toUpperCase() + incomingText.substring(1);

        Long chatId = incomingMessage.getChatId();

        currentChat.expenses.putIfAbsent(capitalizedText, new ArrayList<>());
        currentChat.data = capitalizedText;
        changeState(
